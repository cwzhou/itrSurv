...
SUBROUTINE tfindSplit(nCases, casesIn, nSubj, subjIn, &
  ...
  INTEGER, DIMENSION(1:nCases) :: tcasesSorted, subjSorted
  REAL(dp), DIMENSION(1:nCases) :: sorted_cases
  INTEGER, DIMENSION(1:nv) :: variables
  INTEGER, DIMENSION(:), ALLOCATABLE :: ind, ind_m, indSingles, indSingles_m, leftCases, leftCases_loop, rightCases, rightCases_loop
  ...
  REAL(dp), DIMENSION(1:nCases) :: xSorted, covar_test
...
INTEGER :: iiii
  ...
  if (isPhase2RE) THEN
    ...
    PRINT *, "tsubjects:", tsubjects
    PRINT *, "tsubjind:", tsubjind
  END IF 

  !IF (isPhase1 .OR. isPhase2CR) THEN
  !  IF (nSubj .NE. nCases .OR. nodeSizeSurv .NE. nodeSizeEnd) THEN
  !      PRINT *, "LINE 369 ERROR: NCASES AND NSUBJ SHOULD BE THE SAME AND NODE SIZE SHOULD BE THE SAME"
  !      PRINT *, "nSubj =", nSubj, "nCases =", nCases
  !      PRINT *, "nodeSizeSurv =", nodeSizeSurv, "nodeSizeEnd =", nodeSizeEnd
  !      STOP
  !  END IF
  !END IF
  !print *, "1"
  ...
  DO i = 1, nv
    ...
    IF (nCat(kv) .GT. 1) THEN
      ...
      ELSE
        CALL getCovariate(nCases, casesIn, kv, covar_test)
      END IF
    ELSE
    ...
    ELSE IF (isPhase2RE) THEN
      tcasesSorted = tcases
      CALL qsort4(covar_test, tcasesSorted, 1, nCases)
      !subjSorted = subjIn(tcasesSorted)
      CALL group_and_sort(covar_test, subjIn(tcasesSorted), &
                    tcasesSorted, &
                    delta(tcasesSorted), delta_m(tcasesSorted), &
                    size(covar_test), combArray)
      xSorted = combArray(:, 1) ! xSorted
      personID_og = combArray(:, 2) ! personID_og
      recordID = combArray(:, 3) ! recordID
      dSorted = combArray(:, 4) ! dSorted
      dSorted_m = combArray(:, 5) ! dSorted_m
...
! below is old. can delete.
      ! Phase 2 RE (delta_m = indicator for recurrent event)
    ...
    ! Phase 1
    rUnif = 0.d0
    rUnifSet = -1 !-1 menas doesn't satisify requirements, move to next covariate. this is default and change to 0 if satisfied.
    ! Phase 2
    rUnif_m = 0.d0
    rUnifSet_m = -1 ! -1 menas doesn't satisify requirements, move to next covariate. this is default and change to 0 if satisfied.
    ...
    IF (isPhase2RE) THEN
      ...
      PRINT *, "*************************"
      !CALL find_unique_subjects(uncensoredIndices_m, unique_uncensoredIndices_m1, nUncensored_m1)
      !PRINT *, "test2"
      !PRINT *, "unique_uncensoredIndices_m1"
      !PRINT *,unique_uncensoredIndices_m1
      !PRINT *, "size:",nUncensored_m1 
      IF (print_check) THEN
        PRINT *, "personID_new"
        PRINT *, personID_new
        PRINT *, "dSorted_m"
  ...
  ! cases to right
    ...
    IF ((.NOT. randomSplit) .AND. (ERT .EQ. 1)) THEN
      ...
      IF (uniformSplit .EQ. 0) THEN
        ...
        IF (isPhase2CR .OR. isPhase2RE) THEN
    ...
    ELSE ! Phase 2 (endpoint)
      IF (rUnifSet_m .EQ. -1) CYCLE
    END IF
    ...
    IF (isPhase2RE) PRINT *, "SPLITTING IS DONE"
    if (splitLeft <= 0) then
    ...
    END IF
    IF (isPhase2RE) THEN
      leftCases = personID_new(1:(splitLeft-1))
      rightCases = personID_new(splitLeft:nCases)
    END IF
    ...
    IF (isPhase2RE) THEN
      ...
      !PRINT *, "prsurv"
      !PRINT *, "rows: ", SIZE(prsurv, 1)  ! Number of rows
      !PRINT *, "cols: ", SIZE(prsurv, 2)  ! Number of columns
      !PRINT *, "pr"
      !PRINT *, "rows: ", SIZE(pr, 1)  ! Number of rows
      !PRINT *, "cols: ", SIZE(pr, 2)  ! Number of columns
      !PRINT *, "pr2"
      !PRINT *, "rows: ", SIZE(pr2, 1)  ! Number of rows
      !PRINT *, "cols: ", SIZE(pr2, 2)  ! Number of columns
    END IF 

! delete this once above is checked out.
!if (print_check) THEN
!    if (isPhase1) THEN
!      prl = pr(leftCases,:) ! status change no matter what the status is (censoring, failure, cause specific failure, etc)
!      prr = pr(rightCases,:)
...
!    END IF
!    !PRINT *, "TEST0"
!END IF ! delete this when above is good.
...
if (splitLeft <= 0) then
  ...
  IF (print_check) THEN
    ! below: this gives # events for survival. Uses nt_death.
    PRINT *, "dSorted"
    PRINT *, dSorted
    PRINT *, "splitLeft:", splitLeft
    PRINT *, "dSorted(1:(splitLeft-1))"
    PRINT *, dSorted(1:(splitLeft-1))
    !PRINT *, "spread(dSorted(1:(splitLeft-1)), 2, nt_death) with dimensions (rows:", SIZE(spread(dSorted(1:(splitLeft-1)), 2, nt_death), 1), "and cols: ", SIZE(spread(dSorted(1:(splitLeft-1)), 2, nt_death), 2), ")"
    PRINT *, "spread"
    PRINT *, spread(dSorted(1:(splitLeft-1)), 2, nt_death)
    PRINT *, "prl * &
          & spread(dSorted(1:(splitLeft-1)), 2, nt_death)"
    PRINT *, prl * &
          & spread(dSorted(1:(splitLeft-1)), 2, nt_death)
    PRINT *, "dimension: row:", size(prl * &
          & spread(dSorted(1:(splitLeft-1)), 2, nt_death),1)
    PRINT *, "dimension: col:", size(prl * &
          & spread(dSorted(1:(splitLeft-1)), 2, nt_death),2)
    PRINT *, "sum(prl * &
          & spread(dSorted(1:(splitLeft-1)), 2, nt_death), DIM = 1)"
    PRINT *, sum(prl * &
          & spread(dSorted(1:(splitLeft-1)), 2, nt_death), DIM = 1)
    PRINT *, "after sum: dim: row:", size(sum(prl * &
          spread(dSorted(1:(splitLeft-1)), 2, nt_death), DIM = 1), 1)
    ! Since it's 1D after summing along the first dimension, no need to check size(..., 2)
    PRINT *, "prsurv with dimesnions row: ", size(prsurv,1)
    PRINT *, "prsurv with dimesnions col: ", size(prsurv,2)
    PRINT *, prsurv
    PRINT *, "prl with dimesnions row: ", size(prl,1)
    PRINT *, "prl with dimesnions col: ", size(prl,2)
    PRINT *, prl
    print *, "prsurv 2nd time point"
    PRINT *, prsurv(:,2)

    ! Allocate the temporary array to hold the result (same size as the result of spread operation)
    ALLOCATE(tempArray(SIZE(prl,1), nt_death))
    ! Perform the multiplication and store the result in tempArray
    tempArray = prl * spread(dSorted(1:(splitLeft-1)), 2, nt_death)
    ! Now, print the 5th row of the result
    PRINT *, "row 2 with 5 events?:"
    PRINT *, tempArray(:, 2)
  END IF
  ...
  ELSE IF (isPhase2RE) THEN
    ...
    !PRINT *, "size(leftCases_m)", size(leftCases_m)
...
IF (isPhase2RE) THEN
      ! FIRST: WE WANT TO OBTAIN RE START/STOP INDICES FOR EACH PERSON
        ...
        !DO doi = 1, uniqueCount
        !    PRINT *, "Person ID: ", uniqueID(doi), ", First Index: ", firstIndex(doi), ", Last Index: ", lastIndex(doi)
        !END DO
        !PRINT *, "FirstIndex"
        !PRINT *, firstIndex
        !PRINT *,"lastIndex"
...
DO doi = personID_new(splitLeft), personID_new(splitLeftFinal)
    ...
    ! leftCases_loop = leftCases + add each jth case
    ! rightCases_loop = rightCases - rm jth case 
    leftCases_loop = personID_new(1:(doend))
    rightCases_loop = personID_new(doend+1:nCases)
    ...
    & spread(dSorted(splitLeft_loop+1:nCases), 2, nt), DIM = 1)

    !PRINT *, "prl_loop"
    !PRINT *, prl_loop
    !PRINT *, "prr_loop"
    !PRINT *, prr_loop
    !PRINT *, "splitLeft_loop is ", splitLeft_loop

    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !!!! atRiskLeft_loop and atRiskRight_loop !!!!
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! LATER TO DO: JTH CASE do-loop at-risk:
    pr2l_loop = pr2(leftCases_loop,:) 
    pr2r_loop = pr2(rightCases_loop,:) 
...
IF (isPhase2RE) STOP
  ...
  splitFound = 1
  IF (isPhase2RE) PRINT *, "splitFound: ", splitFound
  PRINT *, "splitVar: ", splitVar
...
subroutine create_new_vector(personID_og, n, personID_new)
    ...
    integer, dimension(n) :: personID_new  ! New array
    integer :: i, currentID
 ...
 subroutine group_and_sort_old(vector1, vector2, vector3, vector4, vector5, n, combinedArray)
        implicit none
        INTEGER, INTENT(IN) :: n
        REAL(dp), DIMENSION(1:n) :: vector1
        INTEGER, DIMENSION(1:n) :: vector2, vector3, vector4, vector5
        REAL(dp), DIMENSION(1:n, 5), INTENT(OUT) :: combinedArray  ! Six columns for combined array
        INTEGER :: i, j
        REAL(dp) :: tempReal
        INTEGER :: tempInt
        ...
        combinedArray(:, 2) = vector2
        combinedArray(:, 3) = vector3
        combinedArray(:, 4) = vector4
        combinedArray(:, 5) = vector5
        !combinedArray(:, 6) = vector6

        ! Sort combinedArray based on vector1 and then vector2
        do i = 1, n-1
            do j = i+1, n
                ! First sort by vector1
                if (combinedArray(i, 1) > combinedArray(j, 1)) then
                    ! Swap vector1
                    tempReal = combinedArray(i, 1)
                    combinedArray(i, 1) = combinedArray(j, 1)
                    combinedArray(j, 1) = tempReal
                    
                    ! Swap vector2
                    tempInt = combinedArray(i, 2)
                    combinedArray(i, 2) = combinedArray(j, 2)
                    combinedArray(j, 2) = tempInt
                    
                    ! Swap vector3
                    tempReal = combinedArray(i, 3)
                    combinedArray(i, 3) = combinedArray(j, 3)
                    combinedArray(j, 3) = tempReal
                    
                    ! Swap vector4
                    tempReal = combinedArray(i, 4)
                    combinedArray(i, 4) = combinedArray(j, 4)
                    combinedArray(j, 4) = tempReal

                    ! Swap vector5
                    tempReal = combinedArray(i, 5)
                    combinedArray(i, 5) = combinedArray(j, 5)
                    combinedArray(j, 5) = tempReal

                    !! Swap vector6
                    !tempReal = combinedArray(i, 6)
                    !combinedArray(i, 6) = combinedArray(j, 6)
                    !combinedArray(j, 6) = tempReal
                else if (combinedArray(i, 1) == combinedArray(j, 1)) then
                    ! Then sort by vector2 if vector1 values are equal
        ...
        PRINT *, "covar_test, subjSorted, tcasesSorted, tcases, delta, delta_m"
        do i = 1, n
            print *, combinedArray(i, :)
        end do

    end subroutine group_and_sort_old

subroutine group_and_sort(vector1, vector2, vector3, vector4, vector5, n, combinedArray)
    implicit none
    INTEGER, INTENT(IN) :: n
    REAL(dp), DIMENSION(1:n) :: vector1
    INTEGER, DIMENSION(1:n) :: vector2, vector3, vector4, vector5
    REAL(dp), DIMENSION(1:n, 5), INTENT(OUT) :: combinedArray  ! Five columns for combined array
    INTEGER :: i, j
    ...
    ! Sort combinedArray based on vector1, vector2, and vector3
    do i = 1, n-1
        do j = i+1, n
            if (combinedArray(i, 1) > combinedArray(j, 1)) then
                call swap_rows(combinedArray, i, j, 5)
            else if (combinedArray(i, 1) == combinedArray(j, 1)) then
                if (combinedArray(i, 2) > combinedArray(j, 2)) then
                    call swap_rows(combinedArray, i, j, 5)
    ...
    ! Output the sorted array
    print *, "Grouped and Sorted Data:"
    PRINT *, "covar_test, subjSorted, tcasesSorted, delta, delta_m"
    do i = 1, n
        print '(F6.1, 1X, I6, 1X, I6, 1X, I6, 1X, I6)', &
...
SUBROUTINE tsurvTree(forestSurvFunc, forestMean, forestSurvProb)
  ...
  DO iTree = 1, nTree
    ...
    jdex_surv = indices_surv
      ...
      !PRINT *, "size(indices)", SIZE(indices)
      !PRINT *, "size(indices_surv_RE)", SIZE(indices_surv_RE)
      !PRINT *, "delta"
      !PRINT *, delta
      !PRINT *, "delta(indices)"
      !PRINT *, delta(indices)
